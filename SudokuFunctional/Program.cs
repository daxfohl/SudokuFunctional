...
800000300".Replace("\r\n", "");
            ...
            sw.Start();
            int[] output = null;
            output = Sudoku.Solve(line.Select(c => int.Parse(c.ToString())).Select(x => x - 1).ToArray()).ToArray();
            Console.WriteLine(sw.ElapsedMilliseconds);
            Console.WriteLine(string.Join(" ", output.Select(i => i.Max() + 1)));
            Console.WriteLine(output.Sum(i => i.PopCount()));
        }
    }
}